...
static const ERD *const SecondaryHeader_PacketDataField_SpacePacket__childrenERDs[1] = {
    &array_SecondaryHeader_PacketDataField_SpacePacket_PacketDataField_SpacePacket__parseSelf
};
...
{
    parse_be_uint8(&instance->Octet, 8, pstate);
    if (pstate->pu.error) return;
}
...
{
    unparse_be_uint8(instance->Octet, 8, ustate);
    if (ustate->pu.error) return;
}
