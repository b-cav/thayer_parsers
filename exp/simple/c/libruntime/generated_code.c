...
static const size_t JSON_number__childrenOffsets[3] = {
    (const char *)&JSON_number__compute_offsets.minus[0] - (const char *)&JSON_number__compute_offsets,
    (const char *)&JSON_number__compute_offsets.zero - (const char *)&JSON_number__compute_offsets,
    (const char *)&JSON_number__compute_offsets.onenine - (const char *)&JSON_number__compute_offsets
};

static const ERD *const JSON_number__childrenERDs[3] = {
    &array_minus_integer_t_JSON_number_ERD,
    &zero_integer_t_ERD,
    &onenine_integer_t_ERD
};
...
JSON_number__parseSelf(JSON_number_ *instance, PState *pstate)
{
    ...
    if (pstate->pu.error) return;
    validate_schema_range(instance->onenine >= 49 && instance->onenine <= 58, "onenine", &pstate->pu);
    if (pstate->pu.error) return;
}
...
JSON_number__unparseSelf(const JSON_number_ *instance, UState *ustate)
{
    ...
    if (ustate->pu.error) return;
    validate_schema_range(instance->onenine >= 49 && instance->onenine <= 58, "onenine", &ustate->pu);
    if (ustate->pu.error) return;
}
