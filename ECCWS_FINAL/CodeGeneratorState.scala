...
class CodeGeneratorState(private val root: ElementBase) {
  ...
  def addBeforeSwitchStatements(): Unit = {
    println("called addBeforeSwitchStatements")
  ...
  private def choiceDispatchField(context: ElementBase): String = {
    println("converting a dispatch field")
  ...
  // Processes basic XPath 2.0 logic to support occursCount expressions with binary data
  // to make decisions and allow variable lengths based on preceding fields
  // Supports:
  // 1) "{ <FIELD> eq xs:hexBinary('<VALUE>')}"
  // 2) "{ if <FIELD> eq xs:hexBinary('<VALUE>') then <RESULTA> else <RESULTB> }"
  // Passes the <FIELD> expression to the cStructFieldAccess function
  // <VALUE> should be a whole-byte value (even number of chars a-f, A-F, 0-9)
  private def xPathLogicHandler(expr: String): String = {
    val xPathIfRegex = """if\(../([a-zA-Z0-9_]+)eqxs:hexBinary\('([0-9a-fA-F]+)'\)\)then(.*)else(.*)"""
    val xPathIfMatcher = Pattern.compile(xPathIfRegex).matcher(expr)

    if (xPathIfMatcher.matches()) {
      val cField = cStructFieldAccess(xPathIfMatcher.group(1).stripPrefix("../"))
      val compBytes = xPathIfMatcher.group(2).grouped(2).toList
      val thenOutput = xPathIfMatcher.group(3)
      val elseOutput = xPathIfMatcher.group(4)

      // Compare each byte of the stored data in the C array to each byte in <VALUE>
      val predicate = compBytes.zipWithIndex.map { case (byte , idx) =>
        s"$cField.array[$idx] == 0x$byte"
      }.mkString(" && ")
      ...
      s"""if ($predicate) {
        ...
        val xPathEqRegex = """([a-zA-Z0-9_]+)eqxs:hexBinary\('([0-9a-fA-F]+)'\)"""
        val xPathEqMatcher = Pattern.compile(xPathEqRegex).matcher(expr)

        if (xPathEqMatcher.matches()) {
          val cStruct = cStructFieldAccess(xPathEqMatcher.group(1).stripPrefix("../"))
          val compBytes = xPathEqMatcher.group(2).grouped(2).toList
          val predicate = compBytes.zipWithIndex.map { case (byte , idx) =>
            s"cStruct.array[$idx] == 0x$byte"
          }.mkString(" && ")
          ...
          s"""if ($predicate) {
            ...
            val cStruct = cStructFieldAccess(expr.stripPrefix("../"))
            cStruct
          }
      }
  }
