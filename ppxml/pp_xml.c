 ...
 *
 *    A copy of the Licence used by Hammer is provided in this repository.
 */
...
static char *build_charset_expanded(const HCharset cs) {
	char buff[MAXCHARSET], *bp;
	char buff1[MAXCHARSET],*ntp;
	int i, min,max,cnt;
	bool range;
	...
	for(cnt=0, min=-1, max=-1, i=0; i<NUMCHARS; i++)
		if(charset_isset(cs,i)) {		/* true if i is in charset */
			*bp=(uint8_t) i;									/* add char to enumerated set */
	...
	for(range=true, i=0; range && i<cnt-1; i++){
		if(((uint8_t)buff[i]+(uint8_t)1)!= ((uint8_t)buff[i+1])){	/* buff contents are not consecutive */
...
static int build_rule(char *lhs,const HCFGrammar *g,const HCFSequence *seq) {
	...
	if(*x!=NULL) {							/* nonempty rhs */
		...
		while(*x) {
			...
			rp->rhs[i++] = rhsp;
			if(i>MAXRHS) {
				printf("error: too many terms on rule %d rhs. %d terms.\n",nextrule,i);
				return -1;
			}
			x++;
		}
